...
export default function SettingsPage({ settings, updateSetting, saveAll, cloud }: Props) {
	...
	useEffect(() => {
		if (!initialSynced && settings !== DEFAULTS) {
			setLocal({ ...settings });
	...
	useEffect(() => {
		(window.clipsta?.listAudioDevices() ?? Promise.resolve([])).then((wasapiDevices) => {
			// Map browser device labels to WASAPI IDs for mic preview
			navigator.mediaDevices.enumerateDevices().then((browserDevices) => {
				...
				for (const d of browserDevices) {
					if (d.kind === "audioinput" && d.label) {
						browserInputByLabel[d.label] = d.deviceId;
					}
				}
				const inputs = wasapiDevices
					.filter((d) => d.kind === "input")
					.map((d) => {
						const browserId = browserInputByLabel[d.name] ?? "";
						return { deviceId: d.id, label: d.name || d.id.slice(0, 32), browserId };
					});
				const outputs = wasapiDevices
					.filter((d) => d.kind === "output")
					.map((d) => ({ deviceId: d.id, label: d.name || d.id.slice(0, 32) }));
	...
	const startMicPreview = useCallback(async (wasapiId: string) => {
		stopMicPreview();
		// Convert WASAPI ID to browser device ID for getUserMedia
		const input = audioInputs.find((d) => d.deviceId === wasapiId);
		const browserDeviceId = input?.browserId || "";
		try {
			const stream = await navigator.mediaDevices.getUserMedia({
				audio: browserDeviceId ? { deviceId: { exact: browserDeviceId } } : true,
			});
			...
			const tick = () => {
				if (!running || !micPreviewRef.current) return;
	...
	// Start/stop mic preview based on audio source and selected device
	useEffect(() => {
		const wantMic = local.audioSource === "mic" || local.audioSource === "both";
		if (wantMic && local.audioInputDeviceId) {
			startMicPreview(local.audioInputDeviceId);
		} else {
			stopMicPreview();
		}
				...
				<Section icon={<Volume2 size={16} />} title="Audio">
					<div className="grid grid-cols-2 gap-4">
						...
						{(local.audioSource === "mic" || local.audioSource === "both") && (
							<>
								<SelectField label="Microphone" value={local.audioInputDeviceId}
									onChange={(v) => {
										update("audioInputDeviceId", v);
										if (v) { startMicPreview(v); } else { stopMicPreview(); }
									}}
										...
										<div className="h-2 bg-muted rounded-full overflow-hidden">
											<div
												...
												style={{
													...
													backgroundColor: micLevel > 0.7 ? "#ef4444" : micLevel > 0.4 ? "#f59e0b" : "#D4F000",
												}}
											/>
										</div>
									</div>
						...
						{(local.audioSource === "desktop" || local.audioSource === "both") && (
							<SelectField label="Desktop Audio Device" value={local.desktopAudioDeviceId}
								onChange={(v) => update("desktopAudioDeviceId", v)}
								options={["", ...audioOutputs.map((d) => d.deviceId)]}
								display={(v) =>
									v === "" ? "System Default" :
									audioOutputs.find((d) => d.deviceId === v)?.label ?? v.slice(0, 16)}
							/>
						)}
					</div>
				</Section>
