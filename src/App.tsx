...
export default function App() {
	...
	// Auto-upload: when a clip is saved and autoUpload+cloudEnabled are on,
	// automatically add the clip to the upload queue.
	...
	useEffect(() => {
		...
		if (settings?.autoUpload && settings?.cloudEnabled) {
			const name = path.split("\\").pop() ?? path.split("/").pop() ?? path;
			window.clipsta?.getFileStats(path).then((stat) => {
				if (stat.size > 0) cloud.addToQueue(path, name, stat.size);
			}).catch(() => {});
		}
	}, [recorder.state.savedPath, settings?.autoUpload, settings?.cloudEnabled, cloud.addToQueue]);

	// Forward main-process [wgc] / [mux] logs to renderer DevTools
	useEffect(() => {
		if (!window.clipsta?.onWgcLog) return;
		window.clipsta.onWgcLog((level, line) => {
			if (level === "error") console.error(line);
			else console.log(line);
		});
		return () => { window.clipsta?.onWgcLog(() => {}); };
	}, []);
			...
			<div className="flex flex-1 overflow-hidden">
				...
				<aside className="w-[200px] flex-shrink-0 bg-[#0d0d0d] border-r border-border flex flex-col py-4">
					...
					<div className="px-3 mt-4 space-y-2">
						...
						<QuickClipButtons onSave1Min={() => recorder.saveClip(60)} onSave5Min={() => recorder.saveClip(300)} active={true} />
					</div>
				</aside>
	...
	return (
		<div className={`rounded-lg px-3 py-2 border ${isRec ? "border-green-800 bg-[#0a1a0a]" : "border-border bg-card"}`}>
			<div className="flex items-center gap-2">
				...
				<span className={`text-xs font-bold ${isRec ? "text-green-400" : "text-text-dim"}`}>
					{isRec ? "BUFFER" : "READY"}
				</span>
				{isRec && (
					<span className="text-xs text-green-600 ml-auto font-mono">{formatDur(duration)}</span>
				)}
			</div>
		</div>
	);
